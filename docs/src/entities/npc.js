...
export class NPCSystem {
  ...
  spawnActorDistrict(){
    ...
    for (let i=0; i<names.length; i++){
      ...
      // assign a "home" door so citizens don't all pile onto the same entrance
      const doors = (this.world && Array.isArray(this.world.doors)) ? this.world.doors.filter(d=>d.target==="building") : [];
      if (doors.length){
        // distribute by index so it's stable and spread out
        n.homeDoor = doors[i % doors.length];
      }
      this.list.push(n);
    }
  }
    ...
    // helper: make a few points around a rect so NPCs don't stack
    const ring = (type, r, k=5) => {
      const cx = r.x + r.w*0.5;
      const cy = r.y + r.h*0.5;
      const radX = Math.max(18, r.w*0.35);
      const radY = Math.max(18, r.h*0.35);
      for (let i=0; i<k; i++){
        const a = (i / k) * Math.PI*2;
        out.push({ type, x: cx + Math.cos(a)*radX, y: cy + Math.sin(a)*radY });
      }
      // also center
      out.push({ type, x: cx, y: cy });
    };

    if (w.park) ring("park", w.park, 7);
    if (w.pool) ring("pool", w.pool, 5);
    if (w.tennis) ring("tennis", w.tennis, 6);

    // stage: if landmarks exist, include several around it
    if (Array.isArray(w.landmarks)){
      const stage = w.landmarks.find(l => l.id === "stage");
      if (stage){
        for (let i=0;i<5;i++){
          out.push({ type:"stage", x: stage.x + (rand(this)*90-45), y: stage.y + (rand(this)*70-35) });
        }
      }
    } else {
      // fallback stage guess
      for (let i=0;i<4;i++) out.push({ type:"stage", x: 1180 + (rand(this)*80-40), y: 520 + (rand(this)*60-30) });
    }

    // mailbox/payphone box
    if (w.box) {
      const bx = w.box.x + w.box.w*0.5, by = w.box.y + w.box.h*0.5;
      for (let i=0;i<4;i++) out.push({ type:"box", x: bx + (rand(this)*70-35), y: by + (rand(this)*70-35) });
      out.push({ type:"box", x: bx, y: by });
    }
...
const door = (n.homeDoor ? { type:"door", x:n.homeDoor.x+n.homeDoor.w*0.5, y:n.homeDoor.y+n.homeDoor.h*0.5, door:n.homeDoor } : byType("door"));
