...
export class Player {
  ...
  draw(ctx, p){
    ...
    // tiny idle breathe only (extra subtle, no "hip wobble" vibe)
    const bob = (!moving && !acting) ? (Math.sin(now * 0.00115) * 0.04) : 0;
    ...
    // hip shift (keep almost still: feet do the talking, not the hips)
    // The old wobble read as "hips swinging". This keeps walk natural.
    let hipX = 0, hipY = 0;
    if (moving && !acting){
      const wob = 0; // set to 1 if you ever want a *tiny* sway
      if (isNS) hipX = stepA ? wob : stepB ? -wob : 0;
      else     hipY = stepA ? wob : stepB ? -wob : 0;
    }
