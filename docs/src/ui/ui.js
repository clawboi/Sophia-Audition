export class UI {
  constructor(root){
    ...
    this.overlayTab = "quests"; // quests | vault | phone | talk
    this.overlayData = {};
...
`;
  ...
  renderOverlay(){
    ...
    if (this.overlayTab === "inventory"){
      const inv = d.inventory || { slots:[], active:0, outfit:"street", outfits:[] };
      const slotName = (it)=>!it ? "EMPTY" : (typeof it==="string" ? it : (it.type||"ITEM")).toUpperCase();
      body.innerHTML = `
        <div class="sec">
          <div class="sec-title">Inventory (Hotkeys 1-3, 0 empty, Q cycle)</div>
          <div class="list">
            ${inv.slots.map((it,idx)=>`
              <div class="item">
                <div>
                  <div class="item-title">Slot ${idx+1}: ${escapeHtml(slotName(it))}</div>
                  <div class="item-sub">${idx===inv.active ? "EQUIPPED" : "Click Equip to hold"}</div>
                </div>
                <div class="item-right">
                  <button class="btn sm" data-equip="${idx}">Equip</button>
                </div>
              </div>
            `).join("")}
            <div class="item">
              <div>
                <div class="item-title">Hands: EMPTY</div>
                <div class="item-sub">${inv.active < 0 ? "EQUIPPED" : "Click Empty Hands"}</div>
              </div>
              <div class="item-right">
                <button class="btn sm" data-equip="-1">Empty</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="sec">
          <div class="sec-title">Clothes</div>
          <div class="muted">Cosmetic outfit changes (foundation). More clothing items later.</div>
          <div class="list">
            ${(inv.outfits||[]).map((o)=>`
              <div class="item">
                <div>
                  <div class="item-title">${escapeHtml((o.label||o.id||"OUTFIT").toUpperCase())}</div>
                  <div class="item-sub">${(o.id===inv.outfit) ? "WEARING" : ""}</div>
                </div>
                <div class="item-right">
                  <button class="btn sm" data-wear="${escapeHtml(o.id)}">${(o.id===inv.outfit) ? "Wearing" : "Wear"}</button>
                </div>
              </div>
            `).join("") || `<div class="muted">No clothes yet.</div>`}
          </div>
        </div>
      `;
      ...
      body.querySelectorAll("[data-equip]").forEach(b=>{
        b.onclick = () => {
          const idx = parseInt(b.getAttribute("data-equip"),10);
          this.onEquipSlot && this.onEquipSlot(idx);
          this.renderOverlay();
        };
      });
      body.querySelectorAll("[data-wear]").forEach(b=>{
        b.onclick = () => {
          const id = b.getAttribute("data-wear");
          this.onWearOutfit && this.onWearOutfit(id);
          this.renderOverlay();
        };
      });
      return;
