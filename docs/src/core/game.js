...
this.inv.applyToPlayer(this.player); // ensures player.held exists
  ...
  update(dt){
    ...
    // V2: NPC life (pause when inside)
    if (this.world?.mode !== "interior") this.npcs.update(dt);
...
if (ax !== 0 || ay !== 0){
  ...
  this.player.faceY = ay;
    ...
    // doors (enter/exit interiors)
    const nearDoor = this.world?.doorNear ? this.world.doorNear(px, py, 62) : null;

    // if we're inside, doors take priority and NPCs are disabled for now
    if (this.world?.mode === "interior" && nearDoor){
      if (this.ui.setPrompt) this.ui.setPrompt("E · Exit");
      if (this.input.pressed("e") && nearDoor.target === "exit"){
        this.world.exitInterior(this.player);
        // respawn NPCs outside (simple reset)
        this.npcs.spawnActorDistrict();
      }
    } else if (nearDoor && this.world?.mode === "outside"){
      const label = (nearDoor.target || "building").toUpperCase();
      if (this.ui.setPrompt) this.ui.setPrompt("E · Enter " + label);
      if (this.input.pressed("e")){
        this.world.enterInterior(nearDoor.target, this.player);
      }
    } else {
    ...
    if (this.ui.setInventory && this.inv){
      const held = this.player.held;
      const heldType = (typeof held === "string") ? held : (held?.type || null);
      this.ui.setInventory({ slotIndex: this.inv.activeSlot, heldType });
    }
  ...
  getOverlayData(){
    ...
    return {
      ...
      inventory: {
        activeSlot: this.inv?.activeSlot ?? 0,
        slots: (this.inv?.slots || []).map(it => {
          if (!it) return null;
          return (typeof it === "string") ? it : (it.type || "item");
        })
      },
      quests,
  ...
  render(){
    ...
    // NPCs (under player) - outside only for now
    if (this.world?.mode !== "interior"){
