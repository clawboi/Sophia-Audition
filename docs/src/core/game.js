...
export class Game {
  constructor({ canvas, ctx, input, save, ui, assets, world }){
    ...
    this.place = "street"; // street | home
    this._streetReturn = null;
    this.lastT = 0;
    ...
    this.player = {
      ...
      // cosmetics
      outfit: "street",
      clothesOwned: ["street","sport","formal","rebel"]
    };
...
this.inv.applyToPlayer(this.player); // ensures player.held exists
    ...
    this.ui.onEquipSlot = (idx) => this.onEquipSlot(idx);
    this.ui.onWearOutfit = (id) => this.onWearOutfit(id);
...
if (this.ui.setInventory){
  ...
  getOverlayData(){
    ...
    return {
      ...
      inventory: {
        slots: (this.inv && this.inv.slots) ? this.inv.slots : [null,null,null],
        active: (this.inv ? this.inv.activeSlot : -1),
        outfit: (this.player.outfit || "street"),
        outfits: (this.player.clothesOwned || []).map(id=>({ id, label:id }))
      },
      rent: {
        text: this.getRentText(now),
        dueAt: this.player.rentDueAt || 0,
        debt: this.player.rentDebt || 0,
        evicted: !!this.player.evicted
      }
    };
  }
  ...
  handleInteract(lm){
    ...
    switch (lm.id){
      ...
      case "park":
        ...
        break;
      case "apt":
        // Home: enter interior + open phone tab
        this.enterHome();
  ...
  onEquipSlot(idx){
    if (!this.inv) return;
    // idx: -1 empty hands, 0..2 slot index
    this.inv.activeSlot = (idx|0);
    this.inv.applyToPlayer(this.player);
    if (this.ui.toast){
      if (idx < 0) this.ui.toast("Hands empty");
      else {
        const it = this.inv.slots[idx];
        const name = it ? ((typeof it==="string") ? it : (it.type||"item")) : "empty";
        this.ui.toast("Equipped: " + String(name).toUpperCase());
      }
    }
  }
  ...
  onWearOutfit(id){
    if (!id) return;
    const owned = this.player.clothesOwned || [];
    if (!owned.includes(id)) return;
    this.player.outfit = id;
    if (this.ui.toast) this.ui.toast("Outfit: " + String(id).toUpperCase());
  }

  enterHome(){
    if (this.player.evicted){
      if (this.ui.toast) this.ui.toast("You're evicted. No home access.");
      return;
    }
    if (this.place === "home") return;
    this._streetReturn = { x:this.player.x, y:this.player.y, area:this.player.area };
    this.place = "home";
    if (this.ui.toast) this.ui.toast("Entered Home");
  }

  exitHome(){
    if (this.place !== "home") return;
    this.place = "street";
    if (this._streetReturn){
      this.player.x = this._streetReturn.x;
      this.player.y = this._streetReturn.y;
      this.player.area = this._streetReturn.area || this.player.area;
    }
    if (this.ui.toast) this.ui.toast("Back outside");
  }
  ...
  render(now){
    ...
    if (this.place === "home"){
      ...
      const rent = this.getRentText(nowMs);
      ctx.fillStyle = "rgba(255,255,255,.9)";
      ctx.font = "14px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial";
      ctx.fillText("HOME", 70, 90);
      ctx.fillStyle = (rent.includes("OVERDUE")) ? "#ff6b6b" : "rgba(255,255,255,.75)";
      ctx.fillText(rent + (this.player.rentDebt ? ("  |  DEBT: $"+ (this.player.rentDebt|0)) : ""), 70, 112);
      ctx.fillStyle = "rgba(255,255,255,.6)";
      ctx.fillText("Press E to exit. TAB for Inventory/Phone.", 70, 134);

      // draw player centered
      const px = Math.round(w*0.5 - this.player.w*0.5);
      const py = Math.round(h*0.62 - this.player.h*0.5);
      const p2 = { ...this.player, x:px, y:py };
      this.playerSprite.draw(ctx, p2, this.input, now);
      return;
    }
