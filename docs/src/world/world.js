  ...
  draw(ctx,cam){
    ...
    if (this.mode === "interior"){
      ...
      for(const r of this.roads){
        drawRoad(ctx, r.x, r.y, r.w, r.h);
      }
      if (this.connector) drawRoad(ctx, this.connector.x,this.connector.y,this.connector.w,this.connector.h);

      // parking
      if (this.parking){
        drawAsphalt(ctx, this.parking.x,this.parking.y,this.parking.w,this.parking.h);
        drawRoad(ctx, this.parkingRoad.x,this.parkingRoad.y,this.parkingRoad.w,this.parkingRoad.h);
...
function drawRoad(ctx, x,y,w,h){
  ...
  const long = (w > h);
  const dashC = "rgba(255,255,255,.22)";
  ctx.fillStyle = dashC;
  if (long && h >= 70){
    const cy = (y + h/2 - 2) | 0;
